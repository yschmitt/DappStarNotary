...
const aaa = async () => {
  const instance = await StarNotary.deployed();
  console.log(account);
  await instance.createStar("test", 0, {from: account});
}
...
const lookupStar = async () => {
  const instance = await StarNotary.deployed();
  const id = document.getElementById("starId0").value;
  console.log(id);
  let responce = await instance.tokenIdToStarInfo(Number(id));
  let owner = await instance.ownerOf(Number(id));
  App.setIdStatus("Owner:" + owner + "      Content:"+responce);
}
//
